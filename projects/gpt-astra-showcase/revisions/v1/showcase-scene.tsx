...
function glyph(parent:Node,kind:string,column:number,time:()=>number){
  ...
  if(kind==='world'||kind==='integration'){
    ...
    block(parent,0,30,125,142,25,color,column===1?'360°':column===0?'UV':'FIX');
    ...
    else block(parent,-10,38,145,133,20,color,column===0?'IMG':'PLAY');
  }else{
    block(parent,-14,41,150,135,25,color,column===0?(kind==='intro'?'8':'INPUT'):column===1?(kind==='intro'?'11':'{ }'):(kind==='outro'?'CHECK':'PLAY'));
...
export function* explanation(view:View2D,index:number,seconds:number){
  ...
  for(let i=0;i<3;i++){
    ...
    if(i<2){root.add(<Line points={[[x+208,27],[x+306,27]]} endArrow arrowSize={16} stroke={P.blue} lineWidth={4} opacity={.6}/>);}
  }
