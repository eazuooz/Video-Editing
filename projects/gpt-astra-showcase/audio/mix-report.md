...
## 현재 v3 — 중간 PPT 없는 믹스 (2026-09-18)

- 결과: `review/astra-no-middle-slides-v3.mp4` / **238.5초 / 14,310프레임 / 1920×1080, 60fps / 70,116,333바이트**
- 중간3초 요약8개를 삭제해24초 단축. 실제 영상221.5초, 도입8초, 마무리9초 유지
- 승인된10개 WAV는 v2와 SHA-256 전부 일치. 재합성·피치·배속 변경 없이 새 시작 시각에 배치
- BGM -28 LUFS, 경쟁하는 원음 -31 LUFS, 승인 내레이션 -16 LUFS 유지. 원음 없는06·07·08은 BGM만 유지
- 곡 끝의 조용한 여운으로 마지막 인사에서 음악이 사라지는 것을 방지: 원곡220초에서 루프를 자르고216~220초에 다음 반복을4초 크로스페이드. 장면 경계와 관계없이 연속 재생
- BGM 전 구간 무음 검사(-60dB, 최소0.5초): 시작0~0.947초의 원곡/페이드 구간 외 중간 무음 없음
- AAC 압축 후 피크 여유를 위해 리미터 ceiling0.77. 전체 **-17.18 LUFS / -1.85 dBTP / LRA14.60 LU**
- 20개 발화/후반부 레이어 표본 확인, 전체 MP4 디코딩 성공. 화면 및 AAC 패킷 해시 일치
- 편집기는 `final-mix-v3.wav`로 연결, 동일 믹스를 PCM으로 디코딩한 파일. 18개 화면과 음소거 해제 재생 확인, 전체 프레임 수 일치 및 최대 경계 오차1프레임
- 한글·영어 SRT 각40개, 같은 시각, 겹침 없음. 마지막237.080초. 기존 v2 자막은 보존
- 근거: `review/audio-v3/mix-report.json`, `review/technical-check-v3.json`, `review/qa-v3-narrated/report.json`
- `publishReady: false`: 원본 재사용 권리와 사람의 전체 청취 승인은 별도

## 보존된 v2 전체 믹스 (2026-09-18)
...
- v2는 `final-mix.wav`와 기존 MP4에 보존. 현재 편집기는 위 v3 믹스로 연결한다. `editorReviewAudio`는 원음만 있는 중간 점검용이며 전체 음성본이 아니다.
