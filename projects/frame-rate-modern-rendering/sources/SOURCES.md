...
- 01–06·08–20은 원본 오디오를 보존하고, 장면별 -23 LUFS를 기준으로 맞춘 뒤
  내레이션이 나올 때 자동으로 낮아지게 믹스한다.
- 07은 선택한 원본 구간의 오디오가 사실상 무음이므로 연속 BGM만 유지한다.
- 19는 Street Fighter II와 Alan Wake 2 소리를 함께 낮춰 섞고, 20은 네 개 패널의 원본
  소리를 각각 낮춘 뒤 합성한다.
- 영상 전체에 `Nimbus — Eveningland`(YouTube Audio Library)를 -28 LUFS 기준으로
  연속 사용한다. 곡 정보와 파일 해시는 `shared/assets/music/youtube-audio-library/`에 기록했다.
- BGM과 게임 원음은 교대하지 않고 함께 재생한다. 원음 구간에는 BGM만 3dB 낮추고,
  곡 반복은 1초 크로스페이드로 연결한다. 최종 측정값은 `audio/mix-report.md`를 따른다.
