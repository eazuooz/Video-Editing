// Port of examples/jump-physics-showcase/explanations.py — only the "why"
// and "how" fields, which are the ones actually rendered on screen.

export type Explanation = {why: string[]; how: string[]};
...
export const EXPLANATIONS: Record<string, Explanation> = {
  ...
  metroid: {
    ...
    how: [
      'float gravity = G_FLOATY;   // 상승·하강 동일 (2.2)',
  ...
  ghosts_n_goblins: {
    ...
    how: [
      'vx = LAUNCH_VX; vy = LAUNCH_VY;   // 점프 시작 시 단 한 번만 결정',
      '',
      'void Update(float dt) {             // 매 프레임',
      '    vy -= gravity * dt;',
      '    x += vx * dt;   // 좌우 입력이 들어와도 절대 반영하지 않음',
      '    y += vy * dt;',
      '}',
    ],
  },
  sf2: {
    ...
    how: [
      '// 공간 경로(모양)는 완전히 고정:',
      'float Shape(float s) { return 4 * H * s * (1 - s); }  // s: 진행도 0~1',
      '',
  ...
  smash_squat: {
    ...
    how: [
      'state = State::SQUAT;        // 착지 프레임부터 짧게 대기',
      'squatTimer -= dt;',
      'if (squatTimer <= 0) {',
  ...
  celeste: {
    ...
    how: [
      ...
      'while (rising) {',
      '    gravity = buttonHeld ? G_RISE : G_CUT;  // 놓는 순간 확 커짐',
      '    velocity -= gravity;',
      '}',
      '// G_CUT(20.0) >> G_RISE(3.0) 이라 놓자마자 상승이 확 꺾인다',
    ],
  },
  megaman: {
    why: [
    ...
    how: [
      ...
      'yVelocity -= gravity;        // 오직 y에만 중력이 작용',
