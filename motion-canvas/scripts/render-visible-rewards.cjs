...
const qa=path.join(root,`shared/output/visible-rewards/visual-qa-v${version}`),out=path.join(root,'shared/output/motion-canvas');fs.mkdirSync(qa,{recursive:true});
...
async function main(){
 ...
 try{
  ...
  if(timing.frames!==frames||timing.scenes.length!==starts.length)throw Error('Runtime timing mismatch '+JSON.stringify(timing));
  const screens=[];
  for(let i=0;i<starts.length;i++)for(const [part,offset] of [['game-start',3],['game-end',16],['diagram',example+9],['diagram-late',durations[i]-2]]){
  ...
  if(audio.muted||!audio.src.includes(path.basename(m.paths.editorAudioMix))||audio.volume<=0)throw Error('Editor mix failed '+JSON.stringify(audio));
  ...
  if(errors.length)throw Error(errors.join('\n'));console.log(`${screens.length} visual checks + editor playback captured.`);
  if(process.argv.includes('--qa-only'))return;
  const pieces=[];
  for(let i=0;i<starts.length;i++){
   const id=String(i+1).padStart(2,'0'),name=`visible-rewards-GRAPHICS-v${version}-`+id,file=path.join(out,name+'.mp4'),firstFrame=Math.round((starts[i]+example)*fps),count=Math.round((durations[i]-example)*fps);
   let cached=false;
   ...
   const folder=version===1?'gameplay':`gameplay-v${version}`;
   const game=path.join(root,`motion-canvas/src/projects/visible-rewards/assets/${folder}/scene${id}.mp4`),silent=path.join(out,`visible-rewards-GAME-v${version}-${id}.mp4`);
  ...
  const target=path.join(out,version===3?'visible-rewards-final.mp4':version===1?'visible-rewards.mp4':`visible-rewards-v${version}.mp4`);
