...
- 예시 기본 길이는 `project.json.editing.exampleSeconds = 19.5`입니다. 템플릿의 회색 예시 준비 화면을 실제 허용 영상으로 교체해야 합니다.
- 전체 음성 패키지는 예시 길이만큼 각 장면 앞을 비우고 KO/EN SRT와 타이밍을 함께 만듭니다. 원음·BGM은 그 구간에 별도 믹스합니다.
